...
public class SubmitAnswersServiceImpl extends RemoteServiceServlet implements SubmitAnswersService {
	...
	public ProblemData submitAnswers(UserWork userWork) {
		...
		LOG.publish(new LogRecord(Level.INFO, "SubmitAnswersServiceImpl#submitAnswers - records deleted: " + result));

		/*
		 * Insert new user work record and trigger insertion of previous answers
		 * record: [0]<-soln_id, [1]<-soln_prob_id, [2]<-soln_username,
		 * [3]<-soln_perm_id, [4]<-num_new_questions_used, [5]<-num_check_used,
